...
class OrbitalLaunchersConfigurationTest {
    ...
    @Nested
    @SpringBootTest(properties = "spring.profiles.active=inspection")
    @DisplayName("using inspection profile")
    class NoLegacyCompliance {
        ...
        @DisplayName("should make available")
        @ParameterizedTest(name = "launcher {0}")
        @ValueSource(strings = {
                "Ariane 5",
                "soyuz",
                "BFR"
        })
        void shouldBeAvailable(String launcherName) {
            assertThat(context).hasBean(launcherName);
        }

        @DisplayName("should not make available")
        @ParameterizedTest(name = "launcher {0}")
        @ValueSource(strings = "energia")
        void shouldNotBeAvailable(String launcherName) {
            assertThat(context).getBean(launcherName).isNull();
        }
    }
}
